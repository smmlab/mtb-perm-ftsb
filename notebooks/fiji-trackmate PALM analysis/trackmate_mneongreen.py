# Process mNeonGreen movies as with mEos3.2 movies but cutting out first 250 frames to remove initial photobleaching
...
def process_directory(image_directory):
	...
	files = glob.glob(image_directory + '514_mNG_stream*.tif')
	...
	for f in files:
		...
		# TODO Add code here to remove first 250 frames !!
		
		# 100 nm pixels and 33 ms frame rate
		imp.getCalibration().setXUnit("nm");
		
		IJ.run(imp, "Slice Remover", "first=1 last=250 increment=1");
		IJ.run(imp, "Properties...", "channels=1 slices=1 frames=250 pixel_width=100 pixel_height=100 voxel_depth=100 frame=[33 ms]");
		...
		# TODO FIND GOOD PARAMETERS FOR mNG
		# Configure detector - We use the Strings for the keys
		settings.detectorFactory = DogDetectorFactory()
		settings.trackerSettings = settings.detectorFactory.getDefaultSettings()
...
for s in subdirectories:
	if s.split('/')[-1][1]=='_':
		...
		print(process_directory(s + '/mNeonGreen_514/')) # for other days
